  ...
  try {
    ...
    app.get('/users/role/:email', async (req, res) => {
      try {
        ...
        console.log("email of users/role/:email", email)
        if (!email) {
    ...
    app.get('/trackings/:trackingId', async (req, res) => {
      ...
      const parcel = await trackingsCollection
        .find({ tracking_id: trackId })
        .sort({ timestamp: 1 })
        .toArray()
    ...
    app.get('/rider/completed-parcels', async (req, res) => {
      try {
        ...
        console.log("email of completed-parcels", email)

        if (!email) {
          return res.status(403).send({ message: "Unauthorized: Rider email not found" });
        }
        ...
        res.status(500).send({ message: "Server error fetching completed parcels" });
      }
    });
